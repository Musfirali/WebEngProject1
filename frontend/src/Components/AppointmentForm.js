  ...
  const handleServiceAndDateChange = async () => {
    try {
      const response = await axios.get(
        `http://localhost:3001/api/appointments/check-availability`,
        {
          params: {
  ...
  return (
    <div>
      <div className="container z-10 mx-auto p-8">
        <h2 className="text-3xl font-bold mt-20 mb-4 bg-yellow-500 text-white py-2 px-4 rounded-lg text-center">
          Book an Appointment
        </h2>
        ...
        >
          ...
          <button
            type="submit"
            className="w-full bg-white rounded-full text-yellow-500 hover:bg-yellow-500 hover:text-white py-3 px-6 font-bold text-xl rounded transition duration-300 border-2 border-yellow-500"
          >
            Book Appointment
          </button>
        </form>
