...
const Cart = () => {
  ...
  return (
    <div className="cart">
      <Navbar />
      <h1 className="mt-28 ml-4 mr-4 text-center font-bold text-3xl bg-yellow-500 p-3 rounded-lg text-white">
        Your Cart
      </h1>
      {cartItems.length === 0 ? (
            ...
            {cartItems.map((item, index) => (
              <tr key={item.id || index}>
                ...
                <td className="border px-4 py-2 text-center">
                  <img
                    src={`http://localhost:3001${item.image}`} // Adjusted the image source to fetch from the backend
                    alt={item.title}
